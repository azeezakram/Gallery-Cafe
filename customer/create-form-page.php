...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $check = "SELECT * FROM customer WHERE username = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $query = "INSERT INTO customer (id, fullname, mobile_no, username, password) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssss", $user_id, $fullname, $mobile_no, $username, $password);
        $stmt->execute();
            ...
            <?php if (isset($_SESSION['username_error'])): ?>
                <script>
                    document.addEventListener("DOMContentLoaded", function() {
                        ...
                        usernameError.innerText = "<?php echo $_SESSION['username_error']; ?>";
                    });
                </script>
